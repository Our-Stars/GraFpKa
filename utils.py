...
class Mol_Des_Fp():
    ...
    # 获取Morgan指纹
    def get_morgan_fp(self, radius=2, nBits=1024):
        fp = GetMorganFingerprintAsBitVect(self.mol, radius=radius, nBits=nBits)
        fp = list(map(float, fp.ToBitString()))
        return fp

    # 获取MACCS指纹
    def get_maccs_fp(self):
        fp = MACCSkeys.GenMACCSKeys(self.mol)
        fp = list(map(float, fp.ToBitString()))
        return fp

    # 获取RDK指纹
    def get_rdk_fp(self):
        fp = RDKFingerprint(self.mol, fpSize=1024)
        fp = list(map(float, fp.ToBitString()))
        return fp

    # 获取Phamacophore ErG指纹
    def get_erg_fp(self, fuzzIncrement=0, maxPath=21, minPath=1):
        fp = GetErGFingerprint(self.mol, fuzzIncrement, maxPath, minPath)
        fp = list(map(float, fp))
        return fp

    # 获取PubChem指纹
    def get_pubchem_fp(self):
        mol = Chem.AddHs(self.mol)
        fp = GetPubChemFPs(mol)
        fp = list(map(float, fp))
        return fp
